...
#define TARGET_RATE_MBPS_7 20480000
...
static long target_rate_bps_7 = 20480000/8;  
static long total_bytes_sent_7= 0;
int main()
{ 
    ...
    int frequency1 = 20000;
    struct timespec ts_sys7;

     clock_gettime(CLOCK_REALTIME, &ts_sys7);

    telemetry_t sending   ={.buffer="hi"};
    sending.timestamp = ts_sys7;
    while (total_bytes_sent_7 < target_rate_bps_7) 
    {  
        ...
        ssize_t bytes_sent_8 = send(sockfd, (void*)&sending, sizeof(telemetry_t), 0);
        if(bytes_sent_8<=0)
        {
        ...
        total_bytes_sent_7+=bytes_sent_8;    
         printf("%s\n",sending.buffer);
    printf("Timestamp sent: %lld seconds, %lld nanoseconds\n", (long long)ts_sys7.tv_sec, (long long)ts_sys7.tv_nsec);
