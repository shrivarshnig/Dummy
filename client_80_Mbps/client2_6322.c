...
#define TARGET_RATE_MBPS_2 62464
typedef struct 
{
...
static long target_rate_bps_2 = 62464/8; 
...
{ 
    ...
    printf("client-2\n");
    socklen_t addr_size;
    struct sockaddr_in ats_sys2_sys2_clientaddress;
    int sockfd;
    struct timespec sleep_time1 = {0};
    struct timespec trig_time;
    addr_size=sizeof(ats_sys2_sys2_clientaddress); 
    ...
    ats_sys2_sys2_clientaddress.sin_family = AF_INET;
    ats_sys2_sys2_clientaddress.sin_addr.s_addr = inet_addr(IP);
    ats_sys2_sys2_clientaddress.sin_port = htons(PORT);

    /*Connect to the client*/
    if (connect(sockfd, (struct sockaddr *)&ats_sys2_sys2_clientaddress,sizeof(ats_sys2_sys2_clientaddress)) == -1) 
    {
    ...
    {  
       if(sigwait(&set, &signum) != 0) {
        ...
        ssize_t bytes_sent_2 = send(sockfd, (void *)&sending, sizeof(telemetry_t), 0);
        if(bytes_sent_2<=0)
        {
        ...
        printf("%s\n",sending.buffer);
        printf("Timestamp sent: %lld seconds, %lld nanoseconds\n", (long long)ts_sys2.tv_sec, (long long)ts_sys2.tv_nsec);
