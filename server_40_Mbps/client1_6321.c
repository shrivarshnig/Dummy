...
#define TARGET_RATE_MBPS_1 5120
typedef struct 
{
...
{ 
    ...
    printf("client-1\n");
    socklen_t addr_size;
    struct sockaddr_in ats_sys1_clientaddress;
    int sockfd;
    struct timespec sleep_time1 = {0};
    struct timespec trig_time;
    char ch;
    
    addr_size=sizeof(ats_sys1_clientaddress); 
    ...
    ats_sys1_clientaddress.sin_family = AF_INET;
    ats_sys1_clientaddress.sin_addr.s_addr = inet_addr(IP);
    ats_sys1_clientaddress.sin_port = htons(port);

    /*Connect to the client*/
    if (connect(sockfd, (struct sockaddr *)&ats_sys1_clientaddress,sizeof(ats_sys1_clientaddress)) == -1) 
    {
    ...
    long int  target_rate_bps_1 = 5120/8;  
    long int total_bytes_sent_1 = 0;
    struct timespec ts_sys1;
    
    clock_gettime(CLOCK_REALTIME, &ts_sys1);
    telemetry_t sending   ={.buffer="hi"};
    sending.timestamp = ts_sys1;
    while (total_bytes_sent_1 < target_rate_bps_1) 
    {
        ...
        ssize_t bytes_sent_1 =send(sockfd,(void *)&sending, sizeof(telemetry_t), 0);
        if(bytes_sent_1 <=0)
        {
        ...
        printf("%s\n",sending.buffer);
    printf("Timestamp sent: %lld seconds, %lld nanoseconds\n", (long long)ts_sys1.tv_sec, (long long)ts_sys1.tv_nsec);
    }
    printf("Sent for port1 %ld bytes at %d Mbps.\n", total_bytes_sent_1, TARGET_RATE_MBPS_1);   
