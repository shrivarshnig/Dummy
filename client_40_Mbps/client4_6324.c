...
#define TARGET_RATE_MBPS_4 3072000
typedef struct 
{
...
static long target_rate_bps_4 = 3072000/8; 
static long total_bytes_sent_4= 0;
...
{ 
    ...
    int frequency1 = 3000; 
    struct timespec ts_sys4;

     clock_gettime(CLOCK_REALTIME, &ts_sys4);

    telemetry_t sending   ={.buffer="hi"};
    sending.timestamp = ts_sys4;
    while (total_bytes_sent_4 < target_rate_bps_4) 
    {  
        if(sigwait(&set, &signum) != 0) {
            perror("sigwait");
            return NULL;
        }
        ssize_t bytes_sent_4 = send(sockfd, (void *)&sending, sizeof(telemetry_t), 0);
        if(bytes_sent_4<=0)
        {
        ...
        printf("%s\n",sending.buffer);
    printf("Timestamp sent: %lld seconds, %lld nanoseconds\n", (long long)ts_sys4.tv_sec, (long long)ts_sys4.tv_nsec);
